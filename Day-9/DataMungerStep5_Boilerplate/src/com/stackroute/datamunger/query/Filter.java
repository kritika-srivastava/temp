...
public class Filter {
  ...
  public boolean evaluateExpression(
    ...
    if (restriction.getCondition().equals("=")) return isEqual(
      fieldValue,
      restriction.getPropertyValue(),
      dataType
    ); else if (restriction.getCondition().matches("!=")) return isNotEqual(
      fieldValue,
      restriction.getPropertyValue(),
      dataType
    ); else if (restriction.getCondition().equals(">")) return isGreaterThan(
      fieldValue,
      restriction.getPropertyValue(),
      dataType
    ); else if (
      restriction.getCondition().equals(">=")
    ) return isGreaterThanOrEqualTo(
      fieldValue,
      restriction.getPropertyValue(),
      dataType
    ); else if (restriction.getCondition().equals("<")) return isLessThan(
      fieldValue,
      restriction.getPropertyValue(),
      dataType
    ); else return isLessThanOrEqualTo(
      fieldValue,
      restriction.getPropertyValue(),
      dataType
    );
  ...
  private String getDateFormat(String date) {
    String format = "";
    if (date.matches("^[0-9]{2}/[0-9]{2}/[0-9]{4}$")) format =
      "dd/mm/yyyy"; else if (
      date.matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$")
    ) format = "yyyy-mm-dd"; else if (
      date.matches("^[0-9]{2}-[a-z]{3}-[0-9]{2}$")
    ) format = "dd-mon-yy"; else if (
      date.matches("^[0-9]{2}-[a-z]{3}-[0-9]{4}$")
    ) format = "dd-mon-yyyy"; else if (
      date.matches("^[0-9]{2}-[a-z]{3,9}-[0-9]{2}$")
    ) format = "dd-month-yy"; else format = "dd-month-yyyy";
    return format;
  }
  ...
  private boolean isEqual(String s1, String s2, String dataType) {
    if (dataType.equals("java.lang.Integer")) {
      ...
      try {
        if (
          formatter.parse(s1).compareTo(formatter.parse(s2)) == 0
        ) return true; else return false;
      } catch (ParseException e) {
        e.printStackTrace();
        return false;
      }
    } else if (dataType.equals("java.util.Object")) return false; else {
  ...
  private boolean isGreaterThan(String s1, String s2, String dataType) {
    if (dataType.equals("java.lang.Integer")) {
      ...
      return (
        Double.parseDouble(s1.toLowerCase()) >
        Double.parseDouble(s2.toLowerCase())
      );
    } else if (dataType.equals("java.util.Date")) {
      SimpleDateFormat formatter = new SimpleDateFormat(getDateFormat(s1));
      try {
